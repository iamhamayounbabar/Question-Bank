...
class PaperApprovalDetail extends Component{
    ...
    componentDidUpdate() {
        if (!this.state.questionLoaded && this.state.yearGroupLoaded
            && this.state.organizationLoaded && this.state.paperTypeLoaded && this.state.paperLoaded) {
            this.loadPaperById();
        }
    }
    ...
    markPaperApprove = async () => {
        try {
            this.setState({ approving: true });
            var response = await this.pService.markPaperApproved(this.state.id);
            let status = response.data.statusNavigation.name;
            this.setState({ status, approving: false, id: response.data.id });
            this.props.navigate('/dashboard/paper-approval-detail/' + response.data.id);
            this.noti.sendNotification(this.noti.types.success, 'Success', 'Paper has been approved.')
        }
        catch (error) {
            this.noti.sendNotification(this.noti.types.error, 'Error', 'Unable to aprroved paper.');
...
export default withRouter(PaperApprovalDetail);